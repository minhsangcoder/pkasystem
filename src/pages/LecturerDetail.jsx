...
import { ArrowLeft, Mail, Phone, MapPin, User, GraduationCap, Building, Briefcase, Award, Calendar, DollarSign, TrendingUp, Upload, X, Check } from 'lucide-react'
...
const LecturerDetail = () => {
  ...
  const fetchLecturerDetail = async (lecturerId) => {
    try {
      setLoading(true)
      const res = await userAPI.getById(lecturerId)
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-gray-500">Đang tải thông tin giảng viên...</div>
      </div>
    )
  }
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        <div className="flex items-start justify-between">
          <div className="flex items-start space-x-4">
            <div className="relative group">
              {lecturer.avatar_url ? (
                ...
                <div className="w-20 h-20 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-full flex items-center justify-center text-white text-2xl font-bold">
                  {lecturer.full_name?.charAt(0)?.toUpperCase() || 'G'}
                </div>
              )}
              <button
            ...
            <div>
              <h1 className="text-3xl font-bold text-gray-900">{lecturer.full_name}</h1>
              <p className="text-sm text-gray-500 mt-1">Mã giảng viên: {lecturer.user_id}</p>
              <div className="flex items-center gap-2 mt-2">
                <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                  lecturer.active === 'Đang làm việc' 
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <p className="text-base text-gray-900 mt-1">{lecturer.gender || '-'}</p>
          </div>
          <div>
            <label className="text-sm font-medium text-gray-500">Học vị</label>
            <p className="text-base text-gray-900 mt-1">
              {Array.isArray(lecturer.degrees) && lecturer.degrees.length > 0
          ...
          <div>
            ...
            <p className="text-base text-gray-900 mt-1">{lecturer.email || '-'}</p>
          </div>
          <div>
            <label className="text-sm font-medium text-gray-500 flex items-center gap-2">
            ...
            <p className="text-base text-gray-900 mt-1">{lecturer.phone || '-'}</p>
          </div>
          <div className="md:col-span-2">
            ...
            <p className="text-base text-gray-900 mt-1">{lecturer.address || '-'}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <p className="text-base text-gray-900 mt-1">{lecturer.OrganizationUnit?.name || '-'}</p>
          </div>
          <div>
            <label className="text-sm font-medium text-gray-500 flex items-center gap-2">
            ...
            <p className="text-base text-gray-900 mt-1">
              {lecturer.Faculty 
                ? `${lecturer.Faculty.faculty_code} - ${lecturer.Faculty.faculty_name}`
                : '-'}
            </p>
          </div>
          <div className="md:col-span-2">
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              {Array.isArray(lecturer.management_positions) && lecturer.management_positions.length > 0
                ? lecturer.management_positions.map((pos, index) => (
                    <span
                      key={index}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div>
            ...
            <p className="text-base text-gray-900 mt-1">
              {lecturer.hire_date 
                ? new Date(lecturer.hire_date).toLocaleDateString('vi-VN')
                : '-'}
            </p>
          </div>
          <div>
            ...
            <p className="text-base text-gray-900 mt-1">
              {lecturer.salary_coefficient ? lecturer.salary_coefficient : '-'}
            </p>
          </div>
          <div>
            <label className="text-sm font-medium text-gray-500 flex items-center gap-2">
            ...
            <p className="text-base text-gray-900 mt-1">
              {lecturer.salary_level ? lecturer.salary_level : '-'}
            </p>
          </div>
        </div>
      </div>
